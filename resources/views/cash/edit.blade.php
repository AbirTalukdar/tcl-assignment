        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <form class="form-wizard" action="{{ route('cash.update', $cashInvestment->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-8">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" id="project_id" name="projectId" required>
                                                @foreach($projects as $project)
                                                    <option value="{{ $project->id }}" {{ $project->id ? 'selected' : '' }}>{{ $project->name }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <!-- Add fields for amount and invest_date -->
                                        <div class="mb-3">
                                            <label for="amount">Amount</label>
                                            <input class="form-control" id="amount" name="amount" type="text" value="{{ $cashInvestment->amount }}" placeholder="Enter Amount" required>
                                            <span class="text-danger"> <b>{{ $errors->first('amount') }}</b></span>
                                        </div>
                                        <div class="mb-3">
                                            <label for="invest_date">Invest Date</label>
                                            <input class="form-control" id="invest_date" name="investDate" type="date" value="{{ $cashInvestment->invest_date }}" required>
                                            <span class="text-danger"> <b>{{ $errors->first('investDate') }}</b></span>
                                        </div>
                                        <!-- End of added fields -->
